...
error_reporting(E_ALL^(E_NOTICE|E_WARNING));
	...
	if(!empty($_POST['submit'])){ // jika tidak kosong
		...
		if(mysql_num_rows($ada)>0){
			...
			$query='INSERT into produk VALUES("'.$kode.'","'.$nama.'","'.$linkGambar.'","'.$deskripsi.'","hubungi kontak","'.$status.'","'.$kategori.'")';
	...
	if(!empty($_POST['update'])){
		...
		$query="UPDATE produk SET namaproduk='".$namaU."',gambar='".$linkGambar."',deskripsi='".$deskripsiU."',harga='hubungi kontak',status='".$statusU."',idkat='".$kategoriU."' WHERE kodeproduk='".$kodeU."'";
		$result=mysql_query($query);
}
...
<ul class="menu">
		...
		<form class="insert" method="post" action="produk.php" enctype="multipart/form-data">
			<table id="tambahproduk">
				...
				<tr></tr>
				<tr><td><label>Kode</label></td><td><input type="text" name="kodeProduk" maxlength="6"></td></tr>
				...
				<?php 
					...
					while($data=mysql_fetch_array($result)){
						...
						if(!empty($_GET['edit'])&&$kode==$_GET['id']){
							...
							echo "<form><table><tr><td><label>Kode Produk</label></td><td><input type='text' size='10px'  name='kode' value='".$kode."'></td></tr>";
	...
	<div class='cp'>
        ...
        <table>
            <tr><td align="center" width="70px"><img src='Contact/WA.png' width='30px' height='30px'></td><td>0878 3858 4777</td></tr>
